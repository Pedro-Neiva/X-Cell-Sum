...
describe('array-util', () => {
  ...
  describe('getLetterRange()', () => {
    it('produces a valid single letter range', () => {
      expect(getLetterRange('Q', 1)).toEqual(['Row Number', 'Q']);
    });

    it('produces a valid letter range starting at A', () => {
      expect(getLetterRange('A', 5)).toEqual(['Row Number', 'A', 'B', 'C', 'D', 'E']);
    });

    it('produces a valid letter range starting at B', () => {
      expect(getLetterRange('B', 5)).toEqual(['Row Number', 'B', 'C', 'D', 'E', 'F']);
    });
